class CommentableContentsController < ApplicationController
  before_action :set_commentable_content, only: [:show, :edit, :update, :destroy]
...
#  end
  ...
  def get_commentable_type
    ...
    def commentable_content_params
      params.fetch(@commentable_type.to_s.underscore.to_sym, {}).permit(:picture,:title, :extract,:content,:commentable_id,:created_by_id,:modified_by_id,:type)
    end
   def log_action
